...
import { useCreateGlobalPromoLink } from '@/hooks/use-global-promo-links';
import type { CreateGlobalPromoLinkResponse } from '@/types/global-promo-link';

interface CreateGlobalPromoLinkModalProps {
	onClose: () => void;
}

export function CreateGlobalPromoLinkModal({
	onClose,
}: CreateGlobalPromoLinkModalProps) {
	const [maxRedemptions, setMaxRedemptions] = useState('');
	const [discountPercent, setDiscountPercent] = useState('');
	const [durationMonths, setDurationMonths] = useState('');
	const [expiresAt, setExpiresAt] = useState('');
	const [result, setResult] = useState<CreateGlobalPromoLinkResponse | null>(
		null,
	);
	const [copied, setCopied] = useState(false);

	const mutation = useCreateGlobalPromoLink();
	...
	async function handleSubmit() {
		...
		try {
			...
			setResult(response);
			toast.success('Link global gerado com sucesso!');
		} catch {
			toast.error('Erro ao gerar link global');
		}
	}
			...
			<div className="relative bg-[#1a1a1d] border border-gray-800 rounded-2xl w-full max-w-lg mx-4 p-6 shadow-2xl">
				<div className="flex items-center justify-between mb-6">
					<h2 className="text-xl font-bold">
						{result ? 'Link global gerado!' : 'Gerar Link Global'}
					</h2>
					<button
						type="button"
						onClick={handleClose}
					...
					<>
						<div className="mb-5 p-3 bg-violet-500/10 border border-violet-500/20 rounded-xl">
							<p className="text-sm text-violet-300">
								<span className="font-medium">Link Global</span> — se aplica a
								todos os produtos.
							</p>
							<p className="text-xs text-violet-400/70 mt-1">
								O cliente escolherá o plano na hora do resgate.
							</p>
						</div>
						...
						<div className="space-y-4">
							<div className="grid grid-cols-2 gap-4">
								<div>
									<label
										htmlFor="global-promo-discount"
										className="block text-sm font-medium text-gray-300 mb-2"
									>
										Desconto (%)
									</label>
									<input
										id="global-promo-discount"
										type="number"
										min={1}
										max={100}
								...
								<div>
									<label
										htmlFor="global-promo-duration"
										className="block text-sm font-medium text-gray-300 mb-2"
									>
										Duração (meses)
									</label>
									<input
										id="global-promo-duration"
										type="number"
										min={1}
							...
							<div>
								<label
									htmlFor="global-promo-max-redemptions"
									className="block text-sm font-medium text-gray-300 mb-2"
								>
									Máximo de usos
								</label>
								<input
									id="global-promo-max-redemptions"
									type="number"
									min={1}
							...
							<div>
								<label
									htmlFor="global-promo-expires-at"
									className="block text-sm font-medium text-gray-300 mb-2"
								>
									Data de expiração{' '}
