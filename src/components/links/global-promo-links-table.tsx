...
import {
	useGlobalPromoLinks,
	useToggleGlobalPromoLinkStatus,
...
export function GlobalPromoLinksTable() {
	const { globalPromoLinks, isLoading } = useGlobalPromoLinks();
	const toggleMutation = useToggleGlobalPromoLinkStatus();
	const [copiedId, setCopiedId] = useState<string | null>(null);

	async function copyLink(token: string, id: string) {
		const url = `${window.location.origin}/global-promo-link/${token}`;
		try {
	...
	if (globalPromoLinks.length === 0) {
		return (
			<div className="text-center py-20">
				<Globe className="w-10 h-10 text-slate-400 dark:text-gray-700 mx-auto mb-4" />
				<p className="text-slate-600 dark:text-gray-400 font-medium">
					Nenhum link global encontrado
				</p>
				<p className="text-slate-500 dark:text-gray-600 text-sm mt-1">
					Links globais criados aparecerão aqui.
				</p>
			</div>
		);
	}
