	...
	return (
		<section>
			<div className="flex justify-between items-end mb-4">
				...
				<Link
					href="/store"
					className="text-violet-600 dark:text-violet-400 hover:text-violet-500 dark:hover:text-violet-300 font-medium text-sm transition-colors"
				>
					Ver todas
				</Link>
			</div>
			...
			{isError || !hasPlans || uniquePlans.length === 0 ? (
				<div className="flex flex-col items-center justify-center py-12 text-center bg-white dark:bg-[#1a1a1d] border border-slate-200 dark:border-gray-800/50 rounded-xl">
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
					{jornadaItems.slice(0, 4).map((item) => (
						<Link
							...
							className="bg-white dark:bg-[#1a1a1d] border border-slate-200 dark:border-gray-800/50 rounded-xl p-4 flex gap-4 hover:bg-slate-50 dark:hover:bg-white/[0.03] hover:border-violet-500/20 transition-all group cursor-pointer"
						>
							<div className="w-28 h-20 rounded-lg overflow-hidden relative shrink-0 bg-violet-500/10 dark:bg-violet-600/20 flex items-center justify-center">
								<BookOpen className="w-8 h-8 text-violet-500 dark:text-violet-400" />
							...
							<div className="flex flex-col justify-between flex-1 py-0.5 min-w-0">
								...
								<div className="mt-2">
									<div className="w-full h-1 bg-slate-200 dark:bg-gray-800/60 rounded-full overflow-hidden">
										<div
					...
					{uniquePlans.slice(0, 4).map((plan) => {
						...
						return plan.slug ? (
							<Link
								...
								className="bg-white dark:bg-[#1a1a1d] border border-slate-200 dark:border-gray-800/50 rounded-xl p-4 flex gap-4 hover:bg-slate-50 dark:hover:bg-white/[0.03] transition-all group"
							>
								<div className="w-28 h-20 rounded-lg bg-violet-500/10 dark:bg-violet-600/20 flex items-center justify-center shrink-0">
									<BookOpen className="w-8 h-8 text-violet-500 dark:text-violet-400" />
								</div>
							...
							<div
								key={plan.id}
								className="bg-white dark:bg-[#1a1a1d] border border-slate-200 dark:border-gray-800/50 rounded-xl p-4 flex gap-4 opacity-60 cursor-not-allowed"
							>
								<div className="w-28 h-20 rounded-lg bg-slate-100 dark:bg-gray-800/40 flex items-center justify-center shrink-0">
									<BookOpen className="w-8 h-8 text-slate-400 dark:text-gray-600" />
								</div>
								<div className="flex flex-col justify-between flex-1 py-0.5">
